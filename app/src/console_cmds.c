...
static int modbus_reg_write_fp(const struct shell *sh, size_t argc, char **argv)
{

    int err = 0;
    const uint8_t unit_id = atoi(argv[1]);
    const uint16_t start_addr = atoi(argv[2]);
    float reg_val = atof(argv[3]);

    err = modbus_write_holding_regs_fp(client_iface, unit_id, start_addr, &reg_val, 1);

    if (err != 0) {
        shell_print(sh, "ERROR: FC06 failed to write floating point value %f to holding register %d on unit %d", reg_val, start_addr, unit_id);
        shell_print(sh, "Error code: %d", err);
    }
    else {
        shell_print(sh, "FC06 Wrote %f to register %d on unit %d", reg_val, start_addr, unit_id);
    }
...
SHELL_STATIC_SUBCMD_SET_CREATE(sub_modbus,
                               ...
                               SHELL_CMD_ARG(write_reg_fp,
                                             NULL,
                                             "<unit_id> <start_addr> <reg_val> - Write floating point holding register",
                                            modbus_reg_write_fp, 3, 3),
